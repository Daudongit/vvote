// https://raw.githubusercontent.com/edward-shen/actix-csrf/master/src/extractor.rs
...
derive_csrf_guarded!(MultipartForm<T>);
        ...
        match self.inner.as_mut().poll(cx) {
            Poll::Ready(Ok(out)) => {
                if let Ok(ref token) = self.csrf_token {
                    dbg!(out.csrf_token().as_ref());
                    dbg!(token.as_ref());
                    return Poll::Ready(Ok(Csrf(out)));
                    // if dbg!(out.csrf_token().as_ref()) == dbg!(token.as_ref()) {
                    //     return Poll::Ready(Ok(Csrf(out)));
                    // }
                }
