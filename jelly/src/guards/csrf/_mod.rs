// https://github.com/edward-shen/actix-csrf/blob/master/src/lib.rs
#![deny(unsafe_code)]
...
impl<S, Rng> Transform<S> for CsrfMiddleware<Rng>
where
    S: Service<Request = ServiceRequest, Response = ServiceResponse, Error = ActixWebError>,
    S::Future: 'static,
    Rng: TokenRng + Clone
{
...
impl<Rng: TokenRng> Inner<Rng> {
    ...
    fn contains(&self, req: &ServiceRequest) -> bool {
        req.match_pattern().map_or_else(
            || {
                self.set_cookie.contains(&(
                    req.method().clone(), req.path().replace("//","/")
                ))
            },
            |p|self.set_cookie.contains(
                &(req.method().clone(), p.replace("//","/"))
            )
        )
    }
}

impl<S, Rng> Service for CsrfMiddlewareImpl<S, Rng>
where
...
#[doc(hidden)]
#[derive(Debug)]
pub enum CsrfMiddlewareImplFuture<S: Service<Request = ServiceRequest, Response = ServiceResponse>> {
